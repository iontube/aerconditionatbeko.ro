...
const prompt = `Scrii articole pe un site romanesc specializat pe aparate de aer conditionat Beko, climatizare, instalare, intretinere si eficienta energetica. Stilul e informativ, practic si directionat spre conversie. NU esti un AI.
...
- Modele Beko reale: BRVPG 120, BRVPG 121, BEHPI 120, BEHPI 121, BEVPI 120, BEVPI 121, BRVPF 090, BRVPF 121, BBFDA 090, BBFDA 120, BBFDA 181 etc.
- Daca nu stii sigur codul exact al unui model, foloseste modele de la alte branduri vandute in Romania: Samsung, LG, Daikin, Mitsubishi, Gree, Vortex, Midea, Cooper&Hunter, Whirlpool — dar NUMAI modele REALE.
- Daca articolul e despre o categorie generala (ex: "12000 BTU"), poti include si modele de la alte branduri pentru comparatie, nu doar Beko.
  ...
  content.items.forEach((item, i) => {
    ...
    itemsHtml += `
          <article id="${productId}" data-review>
            <div data-review-top>
              <span data-section-tag>${stripStrong(productTag)}</span>
  ...
  if (content.comparison && content.comparison.rows && content.comparison.rows.length > 0) {
    ...
    comparisonHtml = `
          <section id="comparatie">
            ...
            <div data-table-wrap>
              <div data-table-hint><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg> Gliseaza pentru mai multe coloane</div>
...
<Layout
  title={\`\${frontmatter.title} | AerConditionatBeko.ro\`}
  ...
  <article>
    <header data-entry-head>
      <div class="shell">
        <nav data-crumbs>
          ...
          <span>/</span>
          <span style="color: var(--color-n-300)">{frontmatter.title}</span>
        </nav>
        <a href={\`/\${frontmatter.categorySlug}/\`} data-entry-cat>{frontmatter.category}</a>
        <h1 data-entry-title>{frontmatter.title}</h1>
        <div data-entry-byline>
    ...
    <div data-entry-body>
      <div data-prose>
        ...
        <div data-outline-mobile id="mobileOutline">
        ...
        <aside data-outline id="sideOutline">
          <div data-outline-label>Cuprins</div>
          <ul>
            ${tocEntries.map(item => `<li><a href="#${item.id}">${item.title}</a></li>`).join('\n              ')}
          </ul>
        </aside>
